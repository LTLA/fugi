---
title: "HiC vignette for fugi package"
output:
  html_document:
    keep_md: yes
...
This vignette shows you how fugi can be used to investigate significant interactions in HiC data that has been analysed using [HOMER](http://homer.salk.edu/homer/) software [1]. fugi can take a HOMER [interaction file](http://homer.salk.edu/homer/interactions/HiCinteractions.html) as input. 
...
```{r}
hic_file <- system.file("extdata", "Seitan2013_WT_100kb_interactions.txt", package="fugi")
...
The set of all regions included in the object can be accessed using `regions()`, or the first and second anchors of the interactions can be accessed using `anchors()`. 
We provide convenience functions `anchorOne` and `anchorTwo` to return the first/second anchors as GRanges.
